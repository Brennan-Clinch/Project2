...
I am going to import the dataset using a relative path and then subset it to only include data from the desired channel.
...
data0<-filter(data0, data_channel_is_socmed == 1)
```
...
*Less than the first quartile shares is "few"
*Between the first and third quartile shares is "some"
*More than the third quartile is "many"
...
library(ggplot2)
data<- data %>% 
  mutate(sharecategory = ifelse(
    shares <quantile(shares,0.25), "few",
                      ifelse(
                        shares<=
                          quantile(shares,0.75),"some",
                             "many")))
...
What we are looking for here is if the mean and standard deviation based on share category is substantially different from category to category or from the overall mean and standard deviation.
...
g+geom_bar(position="dodge")+
  ...
  theme(legend.position = "None")
```
This variable is a measure of how polarizing the title is. Bars further to the right on each graph indicate that the title is more polarizing. We are looking for a difference in where the bars fall in each graph to determine if title polarity has an effect on number of shares.
...
boostFit <- train(shares~num_imgs
                ...
                preProcess = c("center","scale"), 
                tuneGrid =  expand.grid(n.trees = c(25,100), 
