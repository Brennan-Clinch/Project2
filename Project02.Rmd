...
Let's start by creating a new factor variable for the training set which categorizes shares based on the number of them. 
...
Our model for the tech channel is (lambda(shares)~num_imgs
*num_videos*average_token_length*title_sentiment_polarity)
...
Now let's set up our model comparison functions that automate the process.
```{r}
RMSEcv <- unlist(
  sapply(
    sapply(
...
The best performing model in repeated k-folds CV is the `r bestModelName` with an RMSE
of `r bestRMSE` for the training set.

Usually, we would pick the best performing model here to
...
The best performing model on the testing set is the `r bestModelName` with
an RMSE of `r bestRMSE`.
