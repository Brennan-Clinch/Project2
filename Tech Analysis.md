...
Let us first take a look at shares. We created a new variable sharesSumm that provides a 5-number summary of shares for the Technology channel.
...
From the summary, it appears the mean of average word length for the Technology channel was 4.58 and the standard deviation was 0.36.
...
train2 <- train(shares~num_imgs
              ...
              trControl=trainControl(method="cv",number=10))
```
Our model for the Technology channel is lambda(shares) as the response and all interactions of number of images, number of videos, average word length, and title polarity as the predictors.
...
So we first set up vectors of our models to store in a table/data frame for later when looking at our top candidate

``` r
model_Name <- c("OLS", "Poisson Regression", "Random Forest", "Boosted tree")
```
Now let's use our models on the testing set and see how well they do to pick our best model for the Technology channel.
